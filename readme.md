...
- **대기** 탭4 입력완료 후 사내 그룹웨어(FastAPI)로 측정 데이터·PDF 자동 전송 (옵션)
...
1. Python이 설치된 PC에서 `0.처음사용시\드라이버+파이썬패키지+보안경고 통합설치.py` 더블클릭 실행
...
> `0.처음사용시` 설치 도구는 Smart App Control 차단을 피하기 위해 `.bat` → `.py` 로 바꿨다.
...
- `groupware_client.py` (대기 그룹웨어 연동, 옵션)

GUI 요약 (`eco_input_gui.py`):

- 매체: Notebook 탭 **대기** / **수질** — 한쪽만 표시. 아이디·비밀번호·진행바·시작은 공통
- 그룹웨어 연동은 체크박스 없이 `config.ini` `[GROUPWARE] ENABLED=1` 기준으로 자동 전송. 수질은 미적용
- **대기** 안쪽 Notebook: **측정** / **분석** (한 번에 하나만 실행)
  - 측정: 탭1 | 백데이터 / 탭2 | 탭2 PDF
  - 분석: 탭4 | 탭4 PDF
  - 탭을 바꾸면 해당 탭의 선택박스를 모두 켠다. 탭2 끄면 탭2 PDF 자동 OFF, 탭4 끄면 탭4 PDF 자동 OFF
  - 시작 시에는 **지금 열린 측정/분석 탭만** `_main_air` 플래그로 넘긴다 (측정 실행 시 `do_tab4=False`, 분석 실행 시 탭1·탭2·백데이터 OFF)
  - 작업 `2`(백데이터만)이면 분석 탭 비활성, 사이트 입력·PDF 체크는 모두 OFF, 백데이터만 ON, 로그인 칸 비활성
  - 모드1(시료번호) / 모드2(팀+날짜)는 쓰는 입력란만 표시
...
- **수질**은 측정/분석으로 나누지 않음. 옵션(한 줄): 시험의뢰정보 (탭2) / 탭4 / 탭4 PDF (탭4 끄면 PDF OFF)
...
- 창 기본 `860×640`, min `800×560`

대기 GUI → `_main_air()` 플래그 (키 이름 기존과 동일):

| 열린 탭 | `do_tab1` | `do_tab2` | `do_pdf_upload` | `do_backdata` | `do_tab4` | `do_pdf_final` |
|---------|-----------|-----------|-----------------|---------------|-----------|----------------|
| 측정 | 체크 | 체크 | 탭2 ON일 때만 | 체크 | False | False |
| 분석 | False | False | False | False | 체크 | 탭4 ON일 때만 |
| 백데이터만 | False | False | False | True | False | False |

수질 GUI → `_main_water()` 인자는 **§4-1-1** 표와 동일 (탭2+탭4 한 번에 가능).
...
- **그룹웨어 연동**(V3.5): 대기 탭4 분석완료 직후 `groupware_client` → FastAPI. `collected_at`/`measure_date` 필수, 실패 시 자동 재시도·경고 표시. 상세 **§4-1-3**
- **GUI 탭 전환**(V3.6): 대기/수질 Notebook, 대기는 측정·분석 탭으로 분리. `_main_air`/`_main_water` 인자 이름·의미는 그대로

### 4-1-3. 그룹웨어 연동 (대기 전용, V3.5)

사내 FastAPI 그룹웨어에 자가측정 데이터·PDF를 자동 등록한다. **수질은 연동하지 않는다.**

| 항목 | 내용 |
|------|------|
| 적용 조건 | 대기 + 탭4 + **탭4 PDF** ON → `#btnCompSave` 입력완료 성공 직후 |
| 미전송 | 탭4 임시저장만 한 경우, 수질 실행, `config.ini` `ENABLED=0` |
| 설정 | 루트 `config.ini` → `[GROUPWARE]` (`BASE_URL`, `API_TOKEN`, `ENABLED` 등) |
| 핵심 모듈 | `3.py/groupware_client.py` |
| 훅 위치 | `eco_input._main_air()` — `_try_groupware_tab4_sync()` |
...
비산먼지 채취시간: 대기측정기록부 **F17~I17**. 일반: **F23~I23**.
...
**첨부 PDF (`pdf_groupware`)**

| 구분 | 병합 |
|------|------|
| 일반 | 대기측정기록부 + **먼지시료채취기록지**(시트 있으면) |
| 비산먼지 (파일명에 `비산먼지`) | 대기측정기록부 + **교정** |
| 부가 시트 없음 | 대기측정기록부만 |

- 최종 병합본(`p1`: 커버+분석일지+기록부)은 사이트 보관용 — 그룹웨어에는 안 보냄
- 측정인 업2 업로드는 대기측정기록부 단독 유지

**탭4 목록 「상태」확인 (대기·수질 공통)**

- 입력완료 → 목록 복귀 후 RealGrid에서 해당 시료 **시료번호 열 +2칸(상태)** 읽기
- 셀 값은 `.rg-renderer` 텍스트 (예: `측정분석결과 입력완료`)
- 상세 진입 때 이미 시료번호 검색된 상태이므로 **재검색 없이** 읽고, 행이 없을 때만 재검색

**API 순서**

1. `POST /api/reports/sync` — JSON(`data`) + PDF(`file`) 한 번에 (권장)
2. 실패 시 `POST /api/external/report_data` → `POST /api/external/report_pdf`

**전송 실패·경고**

- 네트워크·5xx: 5초 간격 최대 3회 재시도
- 422 거래처 미매칭: 재시도 생략
- 응답 `warnings` / `verify_key` / `status` 콘솔 표시 (`collected_at 미전송` 등)
- 전체 완료 후 실패·경고 요약, GUI 팝업 재전송

**로컬 산출물**

- 전송 기록: `{REVIEW_ROOT}\6.그룹웨어전송\{YYYY}년\{M}월\그룹웨어전송_*.xlsx`
  - `데이터전송` / `PDF전송` / `채취시간` / `유량` / `비고`(짧은 실패 메시지)
...
- **THC_B66 출력**: 결과 Report `THC_B66` 열에 성적서 `입력(분석값)` 시트 B66 값을 기록 (C65=1 CSV 방식에서도 출력)
- **진행률 표시**: GUI 하단에 현재 처리 중인 시료 수와 총 시료 수(진행: X / Y)를 표시하여 대규모 작업 시 진행 상황 확인 가능
- **하드코딩 제거**: 모든 NAS 경로가 `config.ini`를 통해 중앙 관리되도록 개선
- **백데이터 정합 비교** (V2.0): 샘플 CSV/FID 형태 비교를 쓰지 않음. 생성/수정시간 OK인 뒤, 백데이터 생성과 같은 규칙으로 성적서 시트를 다시 뽑아 기존 백데이터와 비교
  - 수분 → `수분량자동측정` ↔ `{시료번호}.csv`
  - THC FID(`C65==1`) → `THC 측정값(FID)` ↔ `{시료번호}.csv`
  - THC PF(`C65!=1`) → `THC 측정값(PF)` ↔ `{시료번호}.FID`
  - 불일치 → `내용불일치` / 추출·읽기 실패 → `내용읽기오류`
  - 구현: `receipt.py` → `backdata_utils.compare_backdata_with_report()`
...
- **파일 추가 UI**: `eco_input_gui`와 동일하게 시료번호만 줄바꿈 표시, 오른쪽에 파일 추가/선택 삭제/전체 비우기 버튼 (드래그&드롭 미지원 PC에만 `DnD 설치` 버튼 표시)
- 입자상 채취기준 검사(sample-3에 `[채취기준미달]`로 표시)
  - 먼지: 채취시간 ≥ 40분 OR 시료채취량 ≥ 0.4 OR 무게(C−D) ≥ 0.005 중 하나라도 충족되면 정상
  - 중금속(8종): 시료채취량 ≥ 1 이면 정상
  - 벤조(a)피렌 (조건 1, `입력(분석값)`): 시료채취량 ≥ 3 OR 시료채취시간 ≥ 240분 중 하나라도 충족되면 정상
  - 벤조(a)피렌 (조건 2, `입력` 시트 — 조건 1과 별도): B6=`원형` 이고 B7>4.5 이면 시료채취시간 ≥ 250분 필요 (채취량으로 조건 1을 통과해도 별도 검사)
  - `시료채취량`·`시료채취시간` 열은 입력(분석값) 1행 헤더 기준으로 자동 검색,
    먼지 채취시간(분)은 측정시작/측정 종료(I/K열) 차이로 계산
  ...
  └─ 2.검토 및 입력프로그램.pyw (통합 런처)
      └─ 3.py\ 안의 선택 도구 GUI 실행
          └─ 핵심 로직 호출
              ...
              ├─ NAS에서 파일 검색/저장
              ├─ 결과 엑셀/PDF/백데이터 생성
              └─ [대기·옵션] 그룹웨어 API 전송 (groupware_client)
```
...
- `groupware_client.py` (대기 그룹웨어 연동)
- `measin_constants.py`
...
eco_input.py
  ...
  └─ 탭4 측정분석결과 입력
      ↓
측정인.kr 반영 (탭4 PDF ON 시 분석완료)
      ↓
최종 PDF 저장
      ↓
[옵션] groupware_client → 사내 그룹웨어 (데이터+PDF, §4-1-3)
      ↓
[옵션] 그룹웨어전송 정리 엑셀 (10.검토\6.그룹웨어전송)
```
...
backdata_utils.py
  ├─ 수분량자동측정 → CSV
  ├─ THC 측정값(FID) → CSV  (C65==1)
  └─ THC 측정값(PF)  → FID  (C65!=1)
      ↓
NAS 저장
```
...
발송대장 검토(`receipt.py`)에서의 검증:

```text
사용여부 → 장비 → 측정시간 범위 → 생성/수정시간
  ↓ (시간 OK일 때만)
성적서에서 기대 내용 재추출 (생성과 동일 규칙)
  ↓
기존 백데이터와 텍스트 비교 → OK / 내용불일치
```
...
- 실행·에러 통합 로그: 프로그램루트 `4.log\{연도}년\{월}월\` (`log_utils` — 구 `_logs\error_log.txt` 폐기)
- 그룹웨어 전송 기록 엑셀: `\\192.168.10.163\측정팀\10.검토\6.그룹웨어전송\{연도}년\{월}월\` (GROUPWARE `EXCEL_OUTPUT_DIR`)
...
### 11-1. 실행·에러 통합 로그 (`4.log`)

- 프로그램루트 `4.log\{YYYY}년\{M}월\{YYYYMMDD_HHMMSS}_{프로그램}.log`
- 실행 세션이 없을 때 예외: 같은 폴더 `YYYYMMDD_error.log`
- GUI LogPanel이 stdout을 가로채도 tee로 `4.log`에 남음 (`gui_common.QueueWriter`)
- 구 `_logs\error_log.txt`는 더 이상 쓰지 않음 (지워도 됨)

### 11-2. GUI 안의 로그 창

대부분의 GUI는 화면 아래 또는 오른쪽에 로그 창이 있다.
콘솔 출력이 로그 창과 `4.log`에 함께 남으므로, 실행 순서를 보려면 둘 중 하나를 보면 된다.
...
**먼저** 프로그램 루트 `config.ini` 의 `[GROUPWARE]` 를 수정한다.

- `ENABLED` — `1`(ON) / `0`(OFF) (GUI 체크박스 없음)
